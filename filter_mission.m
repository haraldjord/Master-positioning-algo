%% fliter mission data and plot path.
...
t_start = 5; % mission 14
t_end = 5;
% t_start = 1; % mission 13
% t_end = 0;
...
k = 1;
t_init = mean([t1(1), t2(1), t3(1)]);
for i=1:(length(t1)-2)
    %pause(0.2);      
%     z = (node.ID1.tagData(i)-5)*0.3922;
    z = 0.5 % dive 14 does not have depth measurements
    T21 = t1(i) - t2(i);
    T31 = t1(i) - t3(i);
    ...
    time = mean([t1(i), t2(i), t3(i)]) - t_init;
    if ~isnan(x)
        BV_raw.x(k) = x;
        BV_raw.y(k) = y;
        BV_raw.z(k) = z;
        BV_raw.t(k) = time;
        k = k+1; % #k valid positions 
    end
end


%% filter position of BV and plot result
n_filt = 10;
BV_filt.x = movmean(BV_raw.x, n_filt);
BV_filt.y = movmean(BV_raw.y, n_filt);
BV_filt.z = movmean(BV_raw.z, n_filt);
BV_filt.t = BV_raw.t;


%% plot filtered in 2D
fig1 = figure(1); %position plot
plot3(BV_filt.x, BV_filt.y, BV_filt.z)
grid on
xlabel("x-direction (East) [m]")
ylabel("y-direction (North) [m]");
zlabel("Depth [m]");
set(gca,'ZDir','Reverse');
text(BV_filt.x(1),BV_filt.y(1),BV_filt.z(1),'start')
text(BV_filt.x(end),BV_filt.y(end),BV_filt.z(end),'end')
...
x_dot = delta_x./delta_t;
y_dot = delta_y./delta_t;
z_dot = delta_z./delta_t;
...
v_magnitude = sqrt(x_dot.^2 + y_dot.^2 + z_dot.^2);

fig2 = figure(2)
q3 = quiver3(x, y, z, x_dot, y_dot, z_dot);
xlabel("x-direction (East) [m]")
ylabel("y-direction (North) [m]");
zlabel("Depth [m]");
set(gca,'ZDir','Reverse');
